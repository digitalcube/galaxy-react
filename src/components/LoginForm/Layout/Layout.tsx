...
export const LoginFormLayout: FC<LoginFormLayoutProps> = ({status, children, inprogressMessageTitle, inprogressMessage}) => {
    const classNames = ["login", "px-4", "py-4", "rounded", "d-flex", "flex-column", "justify-content-center"]
        ...
        <div className={classNames.join(' ')}>
            {status === 'inprogress' ? (
                        <div className="mb-4 pb-2 login-header">
				            ...
				            {inprogressMessage ? <div className="mt-4 font-weight-bold text-center send-email">{inprogressMessage}</div>: null}
                        </div>
            ): children}
        </div>
    )
